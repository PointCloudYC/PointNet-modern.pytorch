...
def cloud_normalize(cloud):
    """normalize a cloud by 1) tranlating to center pt of the cloud bounding box and 2)scaling by w. largest distance

    Args:
        cloud (np tensor): a cloud, (N,3)

    Returns:
        np tensor: normalized cloud, (N,3)
    """
    # Center and rescale point for 1m radius
    xyz_min = np.min(cloud, axis=0) # (1,3)
    xyz_max = np.max(cloud, axis=0) # (1,3)
    cloud -= (xyz_min + xyz_max) / 2
    scale = np.max(np.linalg.norm(cloud, axis=1)) # largest distance
    cloud *= 1.0 / scale

    return cloud


def get_scene_seg_features(input_features_dim, cloud, color, height):
    """obtain point features by combining xyz, height and color

    Args:
        input_features_dim (int):  the dimension of input point features
        cloud (np tensor): a cloud, (N,3)
        color (tensor): color of the cloud, (N,3)
        height (tensor): height of the cloud, (N,)

    Returns:
        tensor: return desired input point features, (N,d) where d=1,3,4,5,6,7
    """
    if input_features_dim == 1:
        ...
        features = torch.cat([color, cloud], -1)
    elif input_features_dim == 7:
        features = torch.cat([color, height, cloud], -1)
    else:
        raise NotImplementedError("error")
    return features.transpose(0, 1).contiguous()


class S3DISSemSeg(data.Dataset):
    def __init__(self, input_features_dim, subsampling_parameter,
        ...
        Args:
            ...
            subsampling_parameter: grid/voxel size for subsampling point clouds.
            in_radius: radius of each input spheres for preparing iteration examples, e.g., 2 m.
        ...
        # specific to S3DIS, e.g., dataset labels/categories
        ...
        # prepare iteration indices including cloud_ind and point_ind
        ...
        if not os.path.exists(filename):
            ...
            for ep in range(self.num_epochs):
                for st in range(self.num_steps):
                    ...
                    # Indices of points in input region; this function return a tuple, [0][0] first element of the tuple's first element
            ...
            with open(filename, 'rb') as f:
                # which cloud, center point and kind of noise to use for preparing a iteration example
        ...
        if not os.path.exists(filename):
            ...
            for points, search_tree in zip(self.clouds_points, self.sub_cloud_trees):
                proj_inds = np.squeeze(search_tree.query(points, return_distance=False)) # 1NN search
            ...
            with open(filename, 'rb') as f:
                # for each raw point in each cloud, the 1NN indice of the corresponding sub cloud, e.g., [[3,4,9,...],...]
                self.projections = pickle.load(f)
                print(f"{filename} loaded successfully")

    def __getitem__(self, idx):
        """ retrieve an iteration example from the epoch*num_steps
        Note: each epoch provides num_steps examples(e.g. 2000), so test loader with batchsize 2 will has 1000 length
        Returns:
            current_points: (N, 3), a point cloud.
            mask: (N, ), 0/1 mask to distinguish padding points, useful for computing CE loss.
        ...
        """
        cloud_ind = self.cloud_inds[idx + self.current_epoch * self.num_steps] # which cloud
        point_ind = self.point_inds[idx + self.current_epoch * self.num_steps] # which center pt
        noise = self.noise[idx + self.current_epoch * self.num_steps] # corresponding noise

        cloud_points = np.array(self.sub_cloud_trees[cloud_ind].data, copy=False) # cloud_points
        center_point = cloud_points[point_ind, :].reshape(1, -1)
        pick_point = center_point + noise.astype(center_point.dtype)
        ...
        original_points = cloud_points[input_inds]
...
if __name__ == "__main__":
    ...
    dset = S3DISSemSeg(4, in_radius=2.0, subsampling_parameter=0.04, num_points=15000,
                    num_steps=2000, num_epochs=600, split='train', transforms=transforms)
    dset_v = S3DISSemSeg(4, in_radius=2.0, subsampling_parameter=0.04, num_points=15000,
                      num_steps=2000, num_epochs=20, split='val', transforms=transforms)

    current_idx=0
    points, mask, features, points_labels, cloud_index, input_inds = dset[current_idx]
    print(points.shape, points)
    print(mask.shape, mask)
    print(features.shape, features)
    print(points_labels.shape, points_labels)
    print(cloud_index)
    print(input_inds.shape, input_inds)
