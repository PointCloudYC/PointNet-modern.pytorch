...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, bottleneck_ratio, radius, nsample, config,
        ...
        if downsample:
            self.maxpool = MaskedMaxPool(npoint, radius, nsample, sampleDl) # TODO?
...
class ResNet(nn.Module):
    ...
    def forward(self, xyz, mask, features, end_points=None):
        ...
        # res1
        features = self.conv1(features) # B 72 N
        features = self.la1(xyz, xyz, mask, mask, features)# B 72 N， aggregate info from nbhood  using LA
        xyz, mask, features = self.btnk1(xyz, mask, features) # B 144 N， aggregate info from nbhood using btnk resnet
