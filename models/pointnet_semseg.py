...
class PointNetSemSeg(nn.Module):
    ...
    def forward(self, xyz, mask, features):
        ...
        if self.feature_transform:
            ...
            point_features= x.transpose(2, 1) # Bx64xN
        else:
            transform_feature = None
            point_features=x # Bx64xN

        # x = F.relu(self.conv_bn3(point_features)) # Bx64xN
        x = F.relu(self.conv_bn4(point_features)) # Bx128xN
        
        # IMPORTANT! in orginal code, add the relu but no relu in other replication(yanx27/Pointnet_Pointnet2_pytorch).
        # x = F.relu(self.conv_bn5(x)) # Bx1024xN
        x = self.conv_bn5(x) # Bx1024xN

        # obtain global features using max pooling
        x,_ = torch.max(x,dim=-1) # Bx1024
        
        # concat 
        x = torch.cat((x.unsqueeze(-1).repeat(1,1,self.num_points),point_features), dim=1) # Bx1088xN
